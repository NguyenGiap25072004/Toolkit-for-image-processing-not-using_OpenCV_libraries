# main_median_filter.py
from image_processing.restoration import median_filter_no_opencv
...
class MedianFilterApp:
    ...
    def create_widgets(self):
        ...
        # Frame for kernel size
        kernel_frame = ttk.Frame(self.root)
        kernel_frame.pack(pady=20, padx=20, fill="x")

        ttk.Label(kernel_frame, text="Kernel Size:", style="TLabel").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ttk.Entry(kernel_frame, textvariable=self.kernel_size, width=10, style="TEntry").grid(row=0, column=1, padx=5, pady=5)
    ...
    def process_image(self):
        ...
        try:
            median_filter_no_opencv(input_path, output_path, kernel_size)
